...
/* ── II · the pipeline chain ───────────────────────────────────────────── */
function Chain({ rise }: { rise: Rise }) {
  return (
    <div className="chn">
      <motion.p className="chn__lead t-body" {...rise(0)}>
        The five-stage pipeline processes a single smartphone image through sequential deep learning models, each addressing a specific aspect of e-waste assessment.
      </motion.p>
      <div className="chn__row">
        ...
        <motion.div className="link link--end" {...rise(6)}>
          <span className="link__step">Final Output</span>
          <h3 className="link__q t-display">Handling Recommendation</h3>
          <p className="link__text t-body">
            The pipeline generates a complete assessment with model identification, condition evaluation, damage analysis, value estimation, and recommended handling pathway.
          </p>
        </motion.div>
      </div>
      <motion.div className="chn__detail" {...rise(7)}>
        <p className="t-body">{research.chainDetail}</p>
      </motion.div>
    </div>
  );
}
  ...
  return (
    <div className="cor">
      <motion.div className="cor__lead" {...rise(0)}>
        <span className="cor__name t-display">Dataset & Experiments</span>
        <p className="t-body">
          Comprehensive dataset collection and experimental evaluation across all classification stages with detailed performance analysis.
        </p>
      </motion.div>
      ...
      <motion.div className="cor__detail" {...rise(5)}>
        <p className="t-body">{research.corpusDetail}</p>
      </motion.div>
    </div>
  );
}
  ...
  return (
    <div className="tr">
      <motion.p className="tr__lead t-body" {...rise(0)}>
        Model performance comparison across all classification stages. ViT-based models consistently outperformed CNN baselines.
      </motion.p>
      ...
      <motion.div className="tr__detail" {...rise(4)}>
        <p className="t-body">{research.trialDetail}</p>
      </motion.div>
    </div>
  );
}
  ...
  return (
    <motion.div className="chart" {...rise(k)}>
      ...
      <div className="chart__legend t-mono">
        <span>
          <i className="chart__swatch chart__swatch--base" /> CNN Baseline
        </span>
        <span>
          <i className="chart__swatch chart__swatch--tuned" /> ViT Model
        </span>
      </div>
    </motion.div>
  );
  ...
  return (
    <div className="vc">
      <motion.blockquote className="vc__prompt" {...rise(0)}>
        <span className="t-label">Pipeline Input</span>
      ...
      <motion.div className="vc__detail" {...rise(3)}>
        <p className="t-body">{research.voiceDetail}</p>
      </motion.div>
    </div>
  );
}
